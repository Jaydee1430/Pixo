...
export function BorderGlow({
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if (card && !isHoveredRef.current) {
        card.style.setProperty("--cursor-angle", `${currentAngle.toFixed(1)}deg`);
      }

      if (progress < 1) {
        animId = requestAnimationFrame(loop);
      } else {
        if (card) card.classList.remove("sweep-active");
      }
    };
